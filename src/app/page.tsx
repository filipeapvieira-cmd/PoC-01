...
import { Leaf, Recycle, ShieldCheck, Zap, Bike } from 'lucide-react';
...
interface DashboardData {
  ...
  councilName: string;
  kpis: {
    greenspace: { value: number; unit: string } | null;
    recycling: { value: number; unit: string } | null;
    cycling: { value: number; unit: string } | null;
    protectedAreas: { value: number; unit: string } | null;
    carbonIntensity: { value: number; unit: string } | null;
  };
...
const BREAKDOWN_COLORS = ['#10b981', '#3b82f6', '#8b5cf6', '#f59e0b', '#84cc16', '#06b6d4'];
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.25rem', marginBottom: '1.75rem' }}>
            <StatCard title="Urban Greenspace" value={data.kpis.greenspace?.value ?? null} unit="green features" icon={Leaf} color="#10b981" />
            <StatCard title="Household Recycling" value={data.kpis.recycling?.value ?? null} unit="%" icon={Recycle} color="#3b82f6"
              trendValue={data.kpis.recycling ? 'Scotland 2021-22 data' : undefined} trend="neutral" />
            <StatCard title="Cycling Routes (OSM)" value={data.kpis.cycling?.value ?? null} unit="routes" icon={Bike} color="#06b6d4" />
            <StatCard title="Protected Nature Sites" value={data.kpis.protectedAreas?.value ?? null} unit="SSSI datasets" icon={ShieldCheck} color="#8b5cf6" />
            <StatCard title="Regional Grid Intensity" value={data.kpis.carbonIntensity?.value ?? null} unit="gCO₂/kWh" icon={Zap} color="#f59e0b" />
          </div>
          ...
          {/* ── Charts Row 2: Greenspace Breakdown ── */}
